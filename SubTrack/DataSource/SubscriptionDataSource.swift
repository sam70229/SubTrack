...
protocol SubscriptionDataSourceProtocol {
    ...
    func fetchSubscriptionsForDate(_ date: Date) throws -> [Subscription]
...
//        let currentYear = calendar.component(.year, from: date)
        ...
        let subscriptions = try self.fetchSubscriptions()
        ...
        return subscriptions.filter { subscription in
            ...
            case .biannually:
                // For biannual, check day and if month is on the 6-month schedule
                let subscriptionMonth = calendar.component(.month, from: subscription.firstBillingDate)
                let monthDifference = (currentMonth - subscriptionMonth + 12) % 12
